// BAARO AI Gateway v1 — régional, multi-fournisseur, clés serveur uniquement.
...
async function loadCountry(admin, userId) {
  try {
    const { data } = await admin.from("profiles").select("country,language").eq("id", userId).maybeSingle();
    return data || {};
  } catch { return {}; }
}
...
export default async function handler(req, res) {
  ...
  if (!limit.ok) { Object.entries(limit.headers || {}).forEach(([k,v]) => res.setHeader(k,v)); return res.status(limit.status).json(limit.body); }
  ...
  if (!Array.isArray(messages) || messages.length < 1 || messages.length > 30) return res.status(400).json({ error: "messages doit contenir entre 1 et 30 éléments" });
  ...
  if (normalizedMessages.some((m) => !m.content.trim())) return res.status(400).json({ error: "Message vide ou invalide" });
  ...
  const provider = chooseProvider({ country, requested: requestedProvider || requestedModel });
  if (!provider) return res.status(503).json({ error: "Aucun fournisseur IA n'est configuré" });
  ...
  try {
    let result;
    if (provider === "n8n") {
      result = await callN8n({ url: process.env.N8N_BAARO_WEBHOOK_URL, secret: process.env.N8N_WEBHOOK_SECRET, sessionId, payload: { messages: normalizedMessages, context: { country, user_id: user.id, language: profile.language || null }, max_tokens: maxTokens, mode: mode || "default", system, model: publicModel || process.env.BAARO_AI_MODEL || null, provider, sessionId } });
    } else if (provider === "anthropic") {
      result = await callAnthropic({ apiKey: process.env.ANTHROPIC_API_KEY, messages: normalizedMessages, system, maxTokens });
    } else {
      const cfg = providerConfig(provider);
      result = await callOpenAICompatible({ base: cfg.base, key: cfg.key, model: publicModel || cfg.model, messages: normalizedMessages, system, maxTokens });
    }
    res.setHeader("X-BAARO-AI-Provider", result.provider || provider);
    res.setHeader("X-BAARO-AI-Country", country || "unknown");
    return res.status(200).json({ reply: result.reply, sessionId: result.sessionId || sessionId, provider: result.provider || provider, model: publicModel || provider, country: country || null });
  } catch (err) {
    return res.status(err.status && err.status < 500 ? err.status : 502).json({ error: "Fournisseur IA temporairement indisponible", provider });
  }
}
