...
import { createPayment, getAvailableProviders } from './paymentProvider';
...
export default function ShopRegistrationForm({ onRegistered }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!name.trim() || !city.trim()) {
      setError(t('shop.required_fields'));
      return;
    }
    if (!pricing || !selectedProvider) {
      setError(t('payment.choose_method'));
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto space-y-4 p-4">
      <h2 className="text-lg font-semibold">{t('shop.register_title')}</h2>
      ...
      {pricing && (
        ...
        >
          {isPremium
            ? t('shop.price_premium', {
                amount: pricing.amount_premium,
                normalAmount: pricing.amount_normal,
                currency,
              })
            : t('shop.price_normal', {
                amount: pricing.amount_normal,
                premiumAmount: pricing.amount_premium,
                currency,
              })}
        </div>
      )}
      ...
      <input
        type="text"
        placeholder={t('shop.city_placeholder')}
        value={city}
      ...
      {providers.length > 1 && (
        <div className="space-y-1">
          <label className="text-sm font-medium">{t('payment.choose_method')}</label>
          {providers.map((p) => (
            <button
              key={p.id}
              type="button"
      ...
      >
        {loading
          ? t('shop.preparing_payment')
          : price
            ? t('shop.pay_and_activate', { amount: price, currency })
            : t('common.loading')}
      </button>

      <p className="text-xs text-gray-400 text-center">
        {t('shop.secure_payment_note')}
      </p>
    </form>
  );
}
