...
## BAARO 2.0 — Production gate

Before production deployment, verify in order:
1. Run all Supabase migrations through `014_follow_column_compatibility.sql`.
2. Confirm `public.follows.followed_id` exists and `following_id` does not.
3. Run `npm ci` then `npm run build`.
4. Configure server-only Supabase and AI secrets; never expose service-role keys to Vite.
5. Keep cash payouts disabled until Stripe Connect (or another payout provider) and an idempotent webhook are deployed.
6. Execute RLS tests with two separate users and an anonymous session.

## Sécurité récompenses — migration 015

Les gains métier ne doivent jamais être accordés sur simple déclaration du client. La migration `015_reward_integrity_security.sql` impose une `reference_id` et vérifie l'événement réel en base avant de créditer les points, avec une contrainte d'idempotence pour empêcher le double paiement d'un même événement.

## BAARO 2.0 — Dependency verification
...
The repository intentionally fails `check:lock` when `package.json` and `package-lock.json` diverge. Regenerate the lockfile with `npm install --package-lock-only --ignore-scripts` from an environment with registry access.

## BAARO 2.0 v13 — AI régional

Voir `docs-BAARO-v13-AI.md` et exécuter `supabase/migrations/019_ai_routing_foundation.sql` après la migration 018.
