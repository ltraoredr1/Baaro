...
function ChannelItem({ channel, isActive, unreadCount = 0, lastMessage, onSelect }) {
  ...
  return (
    <button
      ...
      aria-label={`${isVoice? "Vocal" : "Texte"} ${channel.name}${unreadCount>0? `, ${unreadCount} non lus` : ""}`}
    ...
    >
      <div
        ...
        style={{
          background: isActive? `linear-gradient(135deg, ${C.gold}, #ff8c42)` : 'rgba(255,255,255,0.06)',
      ...
      <div className="flex-1 min-w-0">
        ...
        {lastMessage?.text &&!isVoice && (
          <p className="text-[11px] truncate font-medium opacity-70" style={{ color: C.muted }}>
      ...
      <div className="shrink-0 flex items-center gap-1.5">
        ...
        {isActive && unreadCount === 0 && (
          <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" aria-hidden />
        )}
      </div>
    </button>
  );
