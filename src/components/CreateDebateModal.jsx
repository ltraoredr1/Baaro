...
function randomCode(n=6){ return Math.random().toString(36).substring(2,2+n).toUpperCase(); }
...
export function CreateDebateModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleCreate = async (e) => {
    ...
    if (!titleVal ||!topicVal) { setError("Titre et thème requis."); return; }

    setLoading(true); setError(null);
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const userId = session?.user?.id;
      if (!userId) throw new Error("Tu dois être connecté pour créer un live. Reconnecte-toi.");

      const finalMode = mode === "hybrid"? "video" : mode;
      const inviteCode = randomCode(6).toLowerCase();
      const finalTopic = mode === "hybrid"? `${topicVal} · ⚡ Tout-en-un` : topicVal;

      // 1. Création DIRECTE en base (ça ne peut plus faire Failed to fetch)
      ...
      // 2. Ajout host comme participant
      const { error: partErr } = await supabase
       .from("debate_participants")
       .insert({ room_id: room.id, user_id: userId, role: "host" });
      if (partErr) console.warn("participant warn:", partErr.message);

      // 3. Si audio/video/hybrid → on crée la room Daily EN PLUS, mais sans bloquer si ça échoue
      if (mode!== "text" && API_BASE) {
        try {
          const res = await fetch(`${API_BASE}/api/create-room`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
            body: JSON.stringify({ action: "create-room", userName: "Hôte", title: titleVal, topic: finalTopic, mode: finalMode, inviteCode }),
          });
          const dailyData = await res.json().catch(()=>({}));
          if (dailyData?.roomName) {
            await supabase.from("debate_rooms").update({ daily_room_name: dailyData.roomName }).eq("id", room.id);
            room.daily_room_name = dailyData.roomName;
          }
        } catch (dailyErr) {
          console.warn("Daily API non dispo, salle texte créée quand même:", dailyErr.message);
          // On ne throw pas, la salle est déjà créée
        }
      }

      onSuccess?.(room);
      onClose?.();
      setTitle(""); setTopic(""); setMode("hybrid");
    } catch (err) {
      console.error(err);
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md" onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 border shadow-2xl flex flex-col gap-5 max-h-[92vh] overflow-y-auto" style={{ background: COLORS.surface, borderColor: COLORS.borderGold }}>
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold flex items-center gap-2" style={{ color: COLORS.ivory }}><Zap size={20} style={{ color: COLORS.gold }} /> Nouveau live</h2>
          <button onClick={onClose} className="p-2 rounded-full" style={{ color: COLORS.muted }}><X size={20} /></button>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {TOPIC_SUGGESTIONS.map(t=>(
              <button key={t} type="button" onClick={()=>setTopic(t)} className="text-[10px] px-2.5 py-1 rounded-full border font-medium" style={{ background: topic===t?`${COLORS.teal}22`:COLORS.surface2, borderColor: topic===t?COLORS.teal:COLORS.border, color: topic===t?COLORS.teal:COLORS.muted }}>{t}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {MODES.map(m=>{ const Icon=m.icon; const active=mode===m.id; return (
              <button key={m.id} type="button" onClick={()=>setMode(m.id)} className="flex flex-col items-center gap-1 py-3 px-2 rounded-xl border" style={{ background: active?`${COLORS.gold}18`:COLORS.surface2, borderColor: active?COLORS.gold:COLORS.border, color: active?COLORS.gold:COLORS.muted }}>
                <Icon size={20} /><span className="text-xs font-bold">{m.label}</span><span className="text-[9px] opacity-70 text-center">{m.hint}</span>
              </button>
            )})}
          </div>
          <p className="text-[10px] mt-2 flex items-center gap-1" style={{ color: COLORS.muted }}><Paperclip size={12} /> Fichiers disponibles dans tous les formats</p>
        </div>
        <button type="button" onClick={handleCreate} disabled={!canSubmit} className="w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-40 active:scale-[0.98]" style={{ background: COLORS.gold, color: COLORS.bg }}>
