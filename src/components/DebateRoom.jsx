...
import { ArrowLeft, Hash, Users, MessageSquare, Send, Copy, Check } from "lucide-react";
...
const ChatMessage = memo(function ChatMessage({ msg, currentId }) {
  ...
  return (
    <div className={`flex gap-2 ${isMe? "flex-row-reverse" : "flex-row"}`}>
      <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${isMe? "rounded-tr-sm" : "rounded-tl-sm"}`} style={{ background: isMe? COLORS.gold : isAI? "rgba(167,139,250,0.15)" : COLORS.surface2, color: isMe? "#000" : COLORS.ivory }}>
...
export function DebateRoom({ inviteCode, onBack }) {
    ...
    const init = async () => {
      try {
       ...
       .maybeSingle();
        ...
        if (roomErr ||!roomData) throw new Error("Salle introuvable");
        if (mounted) setRoom(roomData);

        await supabase.from("debate_participants").upsert(
          { room_id: roomData.id, user_id: uid, role: roomData.host_id === uid? "host" : "member", left_at: null },
  ...
  const handleSendMessage = async (e) => {
    ...
    if (error) { setNewMessage(text); console.error(error.message); }
  };

  if (error) return <div className="flex flex-col items-center justify-center h-full p-6" style={{background:COLORS.surface}}><p className="text-red-400 mb-4">{error}</p><button onClick={onBack} className="px-6 py-3 rounded-xl font-bold" style={{background:COLORS.gold}}>Retour</button></div>;
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: COLORS.surface }}>
      <div className="flex items-center gap-3 p-4 border-b" style={{ borderColor: COLORS.border }}>
        <button onClick={onBack} className="p-2 rounded-full" style={{ color: COLORS.ivory }}><ArrowLeft size={20} /></button>
        <div className="flex-1 min-w-0">
          <h2 className="font-bold text-sm truncate" style={{ color: COLORS.ivory }}>{room?.title}</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.map(m=><ChatMessage key={m.id} msg={m} currentId={userId} />)}
      ...
      <form onSubmit={handleSendMessage} className="p-4 border-t flex gap-2" style={{ borderColor: COLORS.border }}>
        ...
        <button type="submit" disabled={!newMessage.trim()} className="p-3 rounded-xl" style={{ background: COLORS.gold, color:"#000" }}><Send size={18} /></button>
      </form>
    </div>
  );
}
